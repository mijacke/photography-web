  ...
  initializeAnimations(() => {
    ...
    if (contentGroup.value) {
      ...
      if (subtitleRef.value) {
        tl.from(subtitleRef.value, { y: 25, autoAlpha: 0, duration: 0.6, ease: 'power3.out' })
      }
      if (titleRef.value) {
        tl.from(titleRef.value, { y: 25, autoAlpha: 0, duration: 0.7, ease: 'power3.out' }, '-=0.3')
      }
      if (bodyRef.value) {
  ...
  <section class="section-padding bg-cream-200">
    <div class="container-wide">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        <!-- Image Side -->
        <div class="relative">
          <div ref="imageWrapper" class="about-image-wrapper aspect-[4/5] overflow-hidden" style="will-change: transform;">
          ...
          <!-- Decorative frame -->
          <div ref="frameRef" class="about-frame absolute -bottom-4 -right-4 w-full h-full border-2 border-warm-400 -z-10 hidden lg:block"></div>
        </div>

        <!-- Text Side -->
        <div ref="contentGroup" class="about-content-group lg:pl-8">
          <p ref="subtitleRef" class="about-subtitle text-accent text-xl md:text-2xl mb-3">
            Ahoj, som
          </p>
          
          <h2 ref="titleRef" class="about-title text-3xl md:text-4xl font-display text-charcoal-900 mb-6">
            Pauli Drahošová
          </h2>
